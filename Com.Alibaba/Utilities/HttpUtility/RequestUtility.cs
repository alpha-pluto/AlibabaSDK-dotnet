...
namespace Com.Alibaba.HttpUtility
{
    ...
    public static class RequestUtility
    {
        ...
        /// <param name="encoding"></param>
        /// <param name="cert">证书，如果不需要则保留null</param>
        /// <param name="timeOut"></param>
        /// <returns></returns>
        public static string HttpGet(string url, CookieContainer cookieContainer = null, Encoding encoding = null, X509Certificate cert = null, int timeOut = Config.TIME_OUT)
        {
            ...
            , int timeOut = Config.TIME_OUT)
        {
            ...
            return HttpPost(url, cookieContainer, ms, null, null, encoding, cert, timeOut);
        }
            ...
            , bool checkValidationResult = false)
        {
            ...
            {
                using (Stream streamOnWebException = we.Response.GetResponseStream())
                {
                    using (StreamReader weReader = new StreamReader(streamOnWebException, encoding ?? Encoding.GetEncoding("utf-8")))
                    {
                        string retString = weReader.ReadToEnd();
                        //Console.WriteLine("*************************************");
        ...
        {
            MemoryStream ms = new MemoryStream();
            await formData.FillFormDataStreamAsync(ms);
            return await HttpPostAsync(url, cookieContainer, ms, null, null, encoding, cert, timeOut);
        }
        ...
        /// <para><![CDATA[可以解决类似这样的问题：假设参数名为 tvshow, 参数值为 Tom&amp;Jerry，如果不编码，可能得到的网址： http://a.com/?tvshow=Tom&amp;Jerry&amp;year=1965 编码后则为：http://a.com/?tvshow=Tom%26Jerry&amp;year=1965 ]]></para>
